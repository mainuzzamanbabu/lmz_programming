...
import DrfVisualizer from '../components/DrfVisualizer';
import CodeBlock from '../components/CodeBlock';
import { DRF_CURRICULUM } from '../data/drfConcepts';
import { DrfConcept } from '../types';

const DrfPage: React.FC = () => {
  const [activeConcept, setActiveConcept] = useState<DrfConcept>(DRF_CURRICULUM.concepts[0]);
  ...
  const activeIndex = DRF_CURRICULUM.concepts.findIndex(c => c.id === activeConcept.id);
  const progress = Math.round(((activeIndex + 1) / DRF_CURRICULUM.concepts.length) * 100);
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <span className="text-sm font-bold text-teal-600">🚀 Django REST Framework</span>
          </div>
        ...
        {isSidebarOpen && (
          <div className="lg:hidden fixed inset-0 z-50 bg-black/50 backdrop-blur-sm" onClick={() => setIsSidebarOpen(false)}>
            <div className="absolute left-0 top-0 bottom-0 w-80 bg-white shadow-2xl p-6 overflow-y-auto" onClick={e => e.stopPropagation()}>
              <h3 className="text-lg font-bold text-slate-900 mb-4">🚀 Django REST Framework</h3>
              {DRF_CURRICULUM.concepts.map((concept, index) => (
                <button
                  key={concept.id}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative items-start">
            ...
            {DRF_CURRICULUM.concepts.map((concept, index) => (
              <button
                ...
                className={`w-full text-left py-3 px-4 rounded-2xl transition-all duration-300 ${
                  activeConcept.id === concept.id
                    ? 'bg-gradient-to-r from-teal-50 to-emerald-50 text-teal-700 font-bold shadow-sm border border-teal-100 scale-[1.02]'
                    : index < activeIndex
          ...
          <div className="lg:col-span-9 space-y-8">
            ...
            <section className="bg-white rounded-[2.5rem] p-8 md:p-12 border border-slate-200 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-br from-teal-100 to-emerald-50 rounded-bl-[5rem] opacity-50"></div>
              <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-12">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-4">
                    <span className="inline-flex items-center px-3 py-1 rounded-full bg-teal-100 text-teal-700 text-xs font-bold ring-1 ring-teal-200">
                      Lesson {activeIndex + 1} of {DRF_CURRICULUM.concepts.length}
                    </span>
                  </div>
                ...
                <div className="w-full md:w-80">
                  <DrfVisualizer type={activeConcept.visualType} />
                </div>
              </div>
            </section>
                ...
                <ul className="space-y-3">
                  {activeConcept.keyPoints.map((point, i) => (
                    <li key={i} className="flex items-start space-x-3 text-sm text-slate-600">
                      <span className="w-6 h-6 rounded-lg bg-teal-100 text-teal-600 text-xs flex items-center justify-center font-bold flex-shrink-0 mt-0.5">
                        {i + 1}
                      </span>
            ...
            <div className="flex items-center justify-between pt-4">
              <button
                onClick={() => activeIndex > 0 && setActiveConcept(DRF_CURRICULUM.concepts[activeIndex - 1])}
              ...
              <button
                onClick={() => activeIndex < DRF_CURRICULUM.concepts.length - 1 && setActiveConcept(DRF_CURRICULUM.concepts[activeIndex + 1])}
                className={`flex items-center space-x-2 px-5 py-3 rounded-2xl text-sm font-bold transition-all ${
                  activeIndex < DRF_CURRICULUM.concepts.length - 1
                    ? 'bg-teal-500 text-white hover:bg-teal-600 shadow-md shadow-teal-200'
                    : 'opacity-30 cursor-not-allowed'
                }`}
                disabled={activeIndex >= DRF_CURRICULUM.concepts.length - 1}
              >
